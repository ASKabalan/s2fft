...
def spectral_folding_torch(fm: torch.tensor, nphi: int,
                           L: int) -> torch.tensor:
    """Folds higher frequency Fourier coefficients back onto lower frequency
...
def spectral_periodic_extension_torch(fm: torch.tensor,
                                      L: int) -> torch.tensor:
    """Extends lower frequency Fourier coefficients onto higher frequency
    coefficients, i.e. imposed periodicity in Fourier space. Based on
...
def healpix_fft_numpy(f: np.ndarray, L: int, nside: int,
    ...
    for t in range(ntheta):
        ...
        if reality and nphi == 2 * L:
            ...
            fm_chunk[nphi // 2:] = np.fft.rfft(np.real(f[index:index + nphi]),
                                               norm="backward")[:-1]
        else:
            fm_chunk = np.fft.fftshift(
                np.fft.fft(f[index:index + nphi], norm="backward"))
        ftm[t] = (fm_chunk if nphi == 2 *
...
def healpix_fft_jax(f: jnp.ndarray, L: int, nside: int,
    ...
    for t in range(nside - 1):
        upper_rows.append(ftm_rows_polar[t][0])
        lower_rows.append(ftm_rows_polar[t][1])
...
def healpix_fft_torch(f: torch.tensor, L: int, nside: int,
    ...
    """
    index = 0
    ftm = torch.zeros(samples.ftm_shape(L, "healpix", nside),
    ...
    for t in range(ntheta):
        ...
        if reality and nphi == 2 * L:
            ...
            fm_chunk[nphi // 2:] = torch.fft.rfft(torch.real(f[index:index +
                                                               nphi]),
                                                  norm="backward")[:-1]
        else:
...
def healpix_ifft_numpy(ftm: np.ndarray, L: int, nside: int,
    ...
    Returns:
        np.ndarray: HEALPix pixel-space array.
    """
    f = np.zeros(samples.f_shape(sampling="healpix", nside=nside),
                 dtype=np.complex128)
    ntheta = ftm.shape[0]
    index = 0
    for t in range(ntheta):
        nphi = samples.nphi_ring(t, nside)
        fm_chunk = ftm[t] if nphi == 2 * L else spectral_folding(
            ftm[t], nphi, L)
        if reality and nphi == 2 * L:
            f[index:index + nphi] = np.fft.irfft(fm_chunk[nphi // 2:],
                                                 nphi,
                                                 norm="forward")
        else:
            f[index:index + nphi] = np.fft.ifft(np.fft.ifftshift(fm_chunk),
                                                norm="forward")
        index += nphi
    return f
...
def healpix_ifft_jax(ftm: jnp.ndarray, L: int, nside: int,
    ...
    def ftm_rows_to_f_chunks(ftm_rows, nphi):
        ...
        if reality and nphi == 2 * L:
            ...
            return jnp.conj(
                jnp.fft.fft(jnp.fft.ifftshift(jnp.conj(fm_chunks), axes=-1),
                            norm="backward"))
        return fm_chunks
    ...
    return jnp.concatenate(
        [f_chunks_polar[t][0]
         for t in range(nside - 1)] + [f_chunks_equatorial.flatten()] +
...
def healpix_ifft_torch(ftm: torch.tensor, L: int, nside: int,
    ...
    """
    f = torch.zeros(samples.f_shape(sampling="healpix", nside=nside),
                    dtype=torch.complex128)
    ntheta = ftm.shape[0]
    index = 0
    for t in range(ntheta):
        ...
        fm_chunk = ftm[t] if nphi == 2 * L else spectral_folding_torch(
            ftm[t], nphi, L)
        if reality and nphi == 2 * L:
            f[index:index + nphi] = torch.fft.irfft(fm_chunk[nphi // 2:],
                                                    nphi,
                                                    norm="forward")
        else:
...
def p2phi_rings_jax(t: jnp.ndarray, nside: int) -> jnp.ndarray:
    ...
    tt = jnp.where(t + 1 > 3 * nside,
                   shift * jnp.pi / (2 * (4 * nside - t - 1)), tt)
